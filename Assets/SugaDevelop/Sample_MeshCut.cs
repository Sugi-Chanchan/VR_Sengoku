...
namespace BLINDED_AM_ME
{
    public class MeshCut
    {
        ...
        static void Cut_this_Face(int submesh, bool[] sides, int index1, int index2, int index3)
        {
            ...
            for (int side = 0; side < 3; side++)
            {
                ...
                if (sides[side])
                {
                    ...
                    if (!didset_left)
                    {
                        ...
                        // つまり、アクセスされる可能性がある   //これをしないと左右どっちかは頂点が1つしかないから[1]がそのまま通ってしまう
            ...
            //ここで右側と左側で同じnew_verticesに頂点を追加しているので, ループが終わった後には切断面の頂点がダブっている.それを後で切断面の判断材料にしている...のか? 
            //切断面以外の頂点についてはこの関数は呼ばれていないので切断面についてだけのループ
            // ---------------------------
            // 右側の処理
        ...
        static void FillCap(List<Vector3> vertices)
        {
            ...
            for (int i = 0; i < vertices.Count; i++)
            {
                ...
                //切断面用のマテリアルを当てるから側面のuvとの連続性はない. あくまで切断面のなかでどこに当たるかを計算してる.
                newUV1 = Vector3.zero;
                ...
                //一回の切断で必ず2つの頂点ができるからこの順番でも大丈夫なのか? → verticesは切断面の周りの頂点しか入ってないから多分そう
                left_side.AddTriangle(
                    new Vector3[]{
                        vertices[i],
