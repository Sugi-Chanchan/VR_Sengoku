...
//できるだけ切断で無駄な頂点が増えないようにしているが一回の処理は若干重い
public class MeshCut : MonoBehaviour
{
    ...
    public static Mesh[] CutMeshRough(GameObject target, Vector3 planeAnchorPoint, Vector3 planeNormalDirection)
    {
        ...
        for (int sub = 0; sub < _targetMesh.subMeshCount; sub++)
        {
            ...
            for (int i = 0; i < indices.Length; i += 3)
            {
                ...
                if (sides) //1番目の頂点のある側に残りの2つのポリゴンを持っていく(切断面はジグザグ)
                {
                    _frontMeshData.AddTriangle(p1, p2, p3, sub);
                }
                else
    ...
    public class MeshData
    {
        ...
        void AddTriangle(int notCutIndex0, int notCutIndex1, Vector3 cutPoint, Vector3 normal, Vector2 uv, int submeshNum)
        {
            ...
            //int trackNum = trackedIndex.IndexOf(notCutIndex0);
            int trackNum = CheckIndex(notCutIndex0);

            if (trackNum == -1)
            {
                //trackedIndex.Add(notCutIndex0);
            ...
            // trackNum = trackedIndex.IndexOf(notCutIndex1);
            trackNum = CheckIndex(notCutIndex1);

            if (trackNum == -1)
            {
               // trackedIndex.Add(notCutIndex1);
            ...
            normals.Add(normal);
            uvs.Add(uv);
            //trackedIndex.Add(-1); //新しく作られた頂点は元のメッシュの頂点indexを持っていないので適当に-1を入れておく(何も入れないとtrackedIndexとverticesの対応が崩れる)
            NonCheck();
        }
        ...
        void AddTriangle(int notCutIndex, Vector3 cutPoint1, Vector3 cutPoint2, Vector3[] normals3, Vector2[] uvs3, int submeshNum)
        {
            ...
            //int trackNum = trackedIndex.IndexOf(notCutIndex);
            int trackNum = CheckIndex(notCutIndex);

            if (trackNum == -1)
            {
                //trackedIndex.Add(notCutIndex);
    ...
    public class MyPlane
    {
        Vector3 anchor;
        Vector3 normal;
        public MyPlane(Vector3 anchorPoint,Vector3 normal)
        {
        ...
        {
            var vec = point - anchor;

            if ((normal.x*vec.x+normal.y*vec.y+normal.z*vec.z) > 0)
            {
                return true;
            }
            else
