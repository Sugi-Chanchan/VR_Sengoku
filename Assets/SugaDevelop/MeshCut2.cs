...
public class MeshCut2 : MonoBehaviour
{
    ...
    /// <returns></returns>
    public static Mesh[] CutMesh(GameObject target, Vector3 planeAnchorPoint, Vector3 planeNormalDirection, bool sewMesh)
    {
        ...
        _planeNormal = Vector3.Scale(scale, target.transform.InverseTransformDirection(planeNormalDirection));
        Vector3 anchor = target.transform.InverseTransformPoint(planeAnchorPoint);
        ...
        for (int sub = 0; sub < _targetMesh.subMeshCount; sub++)
        {
            ...
            int length = indices.Length; //先にintに入れておいたほうが処理が早い???
            for (int i = 0; i < length; i += 3)
            {
                ...
                if (sides[0] == sides[1] && sides[0] == sides[2])//3つとも表側, 3つとも裏側のときはそのまま出力
                {
                    if (sides[0])//表側か裏側かを識別
                    {
    ...
    /// <summary>
    /// <para>gameObjectを雑に切断して2つのMeshにして返します. 切断面がザラザラなのでハイポリにしか使えませんが処理はちょっと早いです.</para>
    /// <para>1つ目のMeshが切断面の法線に対して表側, 2つ目が裏側です.</para>
    /// </summary>
    /// <param name="target">切断対象のgameObject</param>
    /// <param name="planeAnchorPoint">切断面上の1点</param>
    /// <param name="planeNormalDirection">切断面の法線</param>
    ...
    public class MeshData
    {
            ...
            for (int sub = 0; sub < subMeshIndices.Count; sub++)
            {
                ...
                for (int i = 0; i < rectanglesList.Count; i++)
                {
                    ...
                    for (int j = 0; j < triangleList.Count; j++)
                    {
                        ...
                        {
                            if (rectangle.key[0] == triangle.key[0])
                            {
                                ...
                                rectangle.key[0] = triangle.key[1];
                                triangleList.RemoveAt(j);
                                j -= 1;//リストの配列でループしてるので配列を消した場合は辻褄を合わせる

                            }
                            else if (rectangle.key[1] == triangle.key[1])
                            {
                                ...
                                rectangle.key[1] = triangle.key[0];
                                triangleList.RemoveAt(j);
                                j -= 1;//リストの配列でループしてるので配列を消した場合は辻褄を合わせる
                            }
                        }
                ...
                for (int first = 0; first < triangleList.Count; first++)
                {
                    ...
                    for (int second = first + 1; second < triangleList.Count; second++)
                    {
                        ...
                        {
                            if (fTriangle.key[0] == sTriangle.key[1])
                            {
                                ...
                                fTriangle.key[0] = sTriangle.key[0];
                                triangleList.RemoveAt(second);
                                second -= 1;
                            }
                            else if (fTriangle.key[1] == sTriangle.key[0])
                            {
                                ...
                                fTriangle.key[1] = sTriangle.key[1];
                ...
                for (int first = 0; first < rectanglesList.Count; first++)
                {
                    ...
                    for (int second = first + 1; second < rectanglesList.Count; second++)
                    {
                        ...
                        {
                            if (fRectangle.key[0] == sRectangle.key[1])
                            {
                                AddTriangle(
                                ...
                                fRectangle.key[0] = sRectangle.key[0];
                                rectanglesList.RemoveAt(second);
                                second -= 1;
                                goto FINDFRAGMENT3;
                            }
                            else if (fRectangle.key[0] == sRectangle.key[1])
                            {
                                AddTriangle(
                                    fRectangle.notCutIndex0,
                                    fRectangle.notCutIndex1,
                                ...
                                fRectangle.key[1] = sRectangle.key[1];
                                rectanglesList.RemoveAt(second);
                                second -= 1;
        ...
        void AddTriangle(TriangleFragment tri, int submeshNum)
        {
            ...
            SetFragment(p0, L0, tri.key[0], tri.parameters[0], tri.newVertices[0], submeshNum);
            SetFragment(p0, L1, tri.key[1], tri.parameters[1], tri.newVertices[1], submeshNum);
        ...
        void AddTriangle(RectangleFragment rec, int submeshNum)
        {
            ...
            SetFragment(p1, L1, rec.key[1], rec.parameters[1], rec.newVertices[1], submeshNum);
            //ここから2つ目のポリゴン
            SetFragment(p1, L1, rec.key[1], rec.parameters[1], rec.newVertices[1], submeshNum);
            SetFragment(p0, L0, rec.key[0], rec.parameters[0], rec.newVertices[0], submeshNum);
    ...
    public class TriangleFragment//三角ポリゴン. 
    {
        ...
        public TriangleFragment(int _notCutIndex, int _lostIndex0, int _lostIndex1, Vector3[] _newVertices, float[] _parameter, Vector3 _cutLine)
        {
            ...
            cutLine = _cutLine;
            key = new int[2] { (_notCutIndex << slide) + _lostIndex0, (_notCutIndex << slide) + _lostIndex1 };
        }
    }
    ...
    public class RectangleFragment//四角ポリゴン
    {
        ...
        public RectangleFragment(int _notCutIndex0, int _notCutIndex1, int _lostIndex, Vector3[] _newVertices, float[] _parameter, Vector3 _cutLine)
        {
            ...
            parameters = _parameter;
            cutLine = _cutLine;
            key = new int[2] { (_notCutIndex0 << slide) + _lostIndex, (_notCutIndex1 << slide) + _lostIndex };
        }
    }
